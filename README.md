...
1. **领导驾驶舱** - 总预算执行率、六大费用类别对比、工程进度、风险预警
2. **工程项目管理** - 30+子工程CRUD、进度跟踪、里程碑管理
3. **预算科目管理** - 三级科目树形结构、预算分配
4. **支出管理** - 手动录入、Excel批量导入、用友NCC对接
5. **模拟分析中心** - What-if分析、情景对比、敏感性分析（龙卷风图）
6. **预警管理** - 预算超支预警、工期延误预警、消耗速率预警
7. **月度考核报表** - 自动生成、数据导出
8. **用户权限管理** - 管理员/领导/部门用户/普通用户 四级角色
